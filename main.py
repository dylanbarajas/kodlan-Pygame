...
pygame.init()
pygame.mixer.init() # para cargar los audios que se usan
...
explosion_list = []
for i in range(1,13):
    explosion = pygame.image.load(f'explosion/{i}.png') # se crean todas las explosiones con diferente nombre
    explosion_list.append(explosion) # se guardan en el arreglo
...
class Jugador(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pygame.image.load("assets/A1.png") .convert_alpha()
    ...
    def update(self):
        ...
        # para que se mantenga dentro de la pantalla
    ...
    def disparar(self):
        ...
        laser_sonido.play() # se reproduce sonido del laser

class Enemigos(pygame.sprite.Sprite):
    def __init__(self,x,y):
        super().__init__()
        self.image = pygame.image.load("assets/E1.png").convert_alpha()
        self.rect = self.image.get_rect()
    ...
    def update(self):
        ...
        self.rect.x += self.time #los enemigos se moverán más rápido conforme pasa el tiempo
...
class Balas(pygame.sprite.Sprite):
    def __init__(self,x,y):
        super().__init__()
        self.image = pygame.image.load("assets/B2.png").convert_alpha()
        self.rect = self.image.get_rect()
        self.rect.centerx = x
        self.rect.y = y
        self.velocidad = -18
...
class Balas_enemigos(pygame.sprite.Sprite):
    def __init__(self,x,y):
        super().__init__()
        self.image = pygame.image.load("assets/B1.png").convert_alpha()
...
class Explosion(pygame.sprite.Sprite):
    def __init__(self,position):
        ...
        img_scala = pygame.transform.scale(self.image, (20,20)) # se reduce el tamño de la imagen
        self.rect = img_scala.get_rect()
        self.rect.center = position
        self.time = pygame.time.get_ticks()
        self.velocidad_explo = 30 #velocidad de cambio de la imagen
        self.frames = 0
    ...
    def update(self):
        ...
        if tiempo - self.time > self.velocidad_explo:
            self.time = tiempo
            self.frames += 1
            if self.frames == len(explosion_list): # si el número de explosiones ya llegó al límite, se van eliminando
                self.kill()
            else:
...
while run:
    ...
    collides1 = pygame.sprite.groupcollide(grupo_balas_jugador, grupo_enemigos,True,True) #para que desaparezcan los objetos que chocan
    for i in collides1:
        score += 10
    ...
    #colisiones jugador - balas_enemigos
    collides2 = pygame.sprite.spritecollide(player, grupo_balas_enemigos,True) #para que desaparezcan los objetos que chocan
    for j in collides2:
        player.vida -=10
        if player.vida <= 0:
            run = False
